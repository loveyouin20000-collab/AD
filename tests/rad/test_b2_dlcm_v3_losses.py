...
def _batch_payload(model: v3.B2DLCMV3, records: list[dict], categories: list[str]):
    depth_payload = {}
...
def test_only_gt_deployment_uses_smooth_max() -> None:
    records = training.build_hermetic_v3_records()
    ...
    model = v3.B2DLCMV3(seed=17)
    payload = _batch_payload(model, batch, cats)
    _loss, parts = v3.total_dlcm_v3_loss(payload, categories=cats)
    assert parts["aggregation"]["gt_deployment"] == "category_smooth_max"
...
def test_coefficients_and_depth_equal_weight() -> None:
    ...
    model = v3.B2DLCMV3(seed=29)
    payload = _batch_payload(model, batch, [r["category"] for r in batch])
    _loss, parts = v3.total_dlcm_v3_loss(payload, categories=[r["category"] for r in batch])
