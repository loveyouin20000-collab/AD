...
def test_bypass_flags_rejected() -> None:
    with pytest.raises(protocol.B2DLCMV3ProtocolError) as exc:
        protocol.reject_bypass_flags({"force_unlock": True})
    assert exc.value.code == "B2_DLCM_V3_CONTRACT_MISMATCH"
...
def test_real_training_gate() -> None:
    ...
    assert exc.value.code == "B2_DLCM_V3_REAL_TRAINING_NOT_ENABLED"
