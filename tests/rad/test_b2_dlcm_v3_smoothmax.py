...
def test_worse_category_has_larger_gradient() -> None:
    ...
    loss = v3._smooth_max_from_tensors(lb, lc, tau=0.05)
    loss.backward()
...
def test_invalid_tau_fails() -> None:
    with pytest.raises(v3.B2DLCMV3Error) as exc:
        v3.smooth_max_normalized([0.1, 0.2], tau=0.0)
    assert exc.value.code == "B2_DLCM_SMOOTHMAX_INVALID"
