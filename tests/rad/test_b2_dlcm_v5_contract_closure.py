"""V5 contract closure: dual dry-run consistency."""
...
from tests.rad.b2_dlcm_v5_fixtures import contract_config
...
def test_dual_dry_run_permutation_stable() -> None:
    ...
    a = calibration.dry_run_complete_v5_contract_validation(
        config=cfg_a, output_dir="/tmp/v5-closure-a"
    )
    b = calibration.dry_run_complete_v5_contract_validation(
        config=cfg_b, output_dir="/tmp/v5-closure-b"
    )
    for key in FLAGS:
        assert a[key] == b[key]
        if key != "teacher_forward_count":
            ...
            assert a[key] == 0
    assert not Path("/tmp/v5-closure-a").exists()
    assert not Path("/tmp/v5-closure-b").exists()
