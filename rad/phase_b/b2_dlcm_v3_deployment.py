"""B2-05C2 V3 deployment export/load (GT deployment head only)."""
...
class B2DLCMV3DeploymentError(RuntimeError):
...
def _fail(code: str, detail: str) -> NoReturn:
    raise B2DLCMV3DeploymentError(code, detail)


def build_h_deploy(checkpoint: Mapping[str, Any]) -> str:
    payload = {
        "schema_version": "b2_dlcm_v3_h_deploy_v1",
...
def export_v3_deployment_checkpoint(
    model: v3.B2DLCMV3,
    *,
    ...
    for banned in v3.AUXILIARY_HEAD_PREFIXES:
        if any(name.startswith(banned) for name in state):
            _fail("B2_DLCM_V3_CONTRACT_MISMATCH", f"auxiliary head leaked: {banned}")
    embedded = v1_deploy.embed_normalization(normalization_stats)
    trunk = v3.B2DLCMV3DeploymentTrunk(seed=None, initialize=False)
    ...
    checkpoint: dict[str, Any] = {
        "schema_version": "b2_dlcm_v3_deployment_checkpoint_v1",
        "architecture_contract_version": v3.ARCHITECTURE_CONTRACT_VERSION,
        "model_class_id": v3.MODEL_CLASS_ID,
...
class ImmutableV3DLCMInference:
    """Inference wrapper exposing only deployment weights."""

    def __init__(self, trunk: v3.B2DLCMV3DeploymentTrunk) -> None:
        self._trunk = trunk
    ...
    def __getattr__(self, name: str) -> Any:
        if name in {
            ...
            _fail("B2_DLCM_V3_CONTRACT_MISMATCH", f"auxiliary API forbidden: {name}")
