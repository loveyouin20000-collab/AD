"""B2-05C3 V3 training: category-balanced sampler, eligibility, selection, dry-run."""
...
class B2DLCMV4TrainingError(RuntimeError):
...
def _fail(code: str, detail: str) -> NoReturn:
    raise B2DLCMV4TrainingError(code, detail)
    ...
    *,
    ...
    if prior_state is not None:
        if prior_state.sampler_contract_version != SAMPLER_CONTRACT_VERSION:
            _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "sampler contract version mismatch")
...
@dataclass
class EligibleWorstRelativeRegretSelector:
    ...
    def consider(
        ...
        *,
        ...
        if self.best_epoch is None:
            self.best_worst = float(worst_relative_regret)
        ...
        if not self.best_eligible:
            self._accept(epoch, worst_relative_regret, macro_kl, gt_signed, eligible=True)
            self.patience_counter = 0
            return True

        md = float(self.min_delta)
        if float(worst_relative_regret) < float(self.best_worst) - md:
            self._accept(epoch, worst_relative_regret, macro_kl, gt_signed, eligible=True)
            self.patience_counter = 0
            return True
        if abs(float(worst_relative_regret) - float(self.best_worst)) <= md:
...
def select_canonical_seed_uniform_relative(
    seed_results: Sequence[Mapping[str, Any]],
    *,
    min_delta: float = MIN_DELTA,
) -> int:
    if not seed_results:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "empty seed results")
    ...
    for row in pool:
        seed = int(row["seed"])
        worst = float(row["worst_relative_regret"])
...
def calibration_metrics_uniform_relative(
    model: v3.B2DLCMV4,
    ...
    *,
    ...
    relative = {c: float(cat_means[c]) - float(uni_means[c]) for c in cat_means}
    ...
    return {
        "worst_relative_regret": worst,
        "per_category_relative_regret": relative,
        "macro_kl": macro,
        "gt_signed": signed,
        "per_category_gt_kl": cat_means,
        "per_category_uniform_gt_kl": uni_means,
...
def build_hermetic_v4_records(*, map_hw: tuple[int, int] = (8, 8)) -> list[dict[str, Any]]:
...
def dry_run_complete_v4_contract_validation(
    *,
    ...
    if list(config.get("candidate_layers", [])) != [6, 12, 18, 24]:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "candidate_layers mismatch")
    if list(config.get("prediction_depths", [])) != [12, 18, 24]:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "prediction_depths mismatch")
    if abs(float(config.get("smoothmax_tau", -1)) - v3.SMOOTHMAX_TAU) > 0:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "smoothmax_tau must be 0.05")
    return dry_run_v4_summary(config=config, seed=seed, output_dir=output_dir)


def dry_run_v4_summary(
    *,
    config: Mapping[str, Any],
    seed: int,
    output_dir: Path | str,
    ...
    if config.get("real_training_enabled") is not False:
        _fail("B2_DLCM_V4_REAL_TRAINING_NOT_ENABLED", "dry-run requires real_training_enabled=false")
    if config.get("contract_stage") != "b2_05c3a":
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "contract_stage must be b2_05c3a")

    records = build_hermetic_v4_records()
    train = [r for r in records if r["split"] == "training"]
    calibration = [r for r in records if r["split"] == "calibration"]
    model = v3.B2DLCMV4(seed=int(seed))
    ...
    if again != batches:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "sampler not reproducible")
    ...
    loss, parts = v3.total_dlcm_v4_loss(depth_payload, categories=categories, tau=v3.SMOOTHMAX_TAU)
    if not bool(torch.isfinite(loss)):
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "dry-run loss nonfinite")
    if parts["aggregation"]["gt_deployment"] != "uniform_relative_smooth_max":
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "gt deploy aggregation mismatch")
    metrics = calibration_metrics_uniform_relative(model, calibration)
    selector = EligibleWorstRelativeRegretSelector()
    selector.consider(
        epoch=0,
        worst_relative_regret=metrics["worst_relative_regret"],
    ...
    if any(any(k.startswith(b) for k in deploy_state) for b in v3.AUXILIARY_HEAD_PREFIXES):
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "auxiliary leaked into deployment")
    ...
    return {
        ...
        "v3_immutable": v3.v3_immutable_identity(),
...
def _cpu_identity_model_v4(model: v3.B2DLCMV4) -> v3.B2DLCMV4:
    clone = v3.B2DLCMV4(
        seed=None,
...
def run_v4_contract_training(
    *,
    ...
    if output_root.exists() and any(output_root.iterdir()) and not allow_existing_output:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "output root must be empty/fresh")
    ...
    if environment_contract is None:
        ...
        if env_path.is_file():
            ...
            if v1_train.environment_contract_sha256(existing) != v1_train.environment_contract_sha256(
                ...
                _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "environment contract drifted")
        else:
            v1_train.persist_environment_contract(env_path, env)

    seed_dir = output_root / f"seed_{seed}"
    if seed_dir.exists():
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", f"seed directory already exists: seed_{seed}")
    ...
    if len(by_split["training"]) != 16 or len(by_split["calibration"]) != 8:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "expected 16/8 training/calibration")
    if len(by_split["evaluation"]) != 8:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "expected 8 evaluation placeholders")
    ...
    if device != "cpu":
        ...
        assert isinstance(moved, v3.B2DLCMV4)
        model = moved
    ...
    metrics0 = calibration_metrics_uniform_relative(model, calibration, device=device_t)
    teacher0 = _teacher_alloc_kl_macro(model, calibration, device=device_t)
    selector.consider(
        epoch=0,
        worst_relative_regret=metrics0["worst_relative_regret"],
    ...
    record0 = v1_train.scientific_epoch_record(
        epoch=0,
        primary=metrics0["worst_relative_regret"],
        secondary=metrics0["macro_kl"],
        total_loss=0.0,
        global_optimizer_step=0,
        sample_ids=train_ids,
    )
    record0["worst_relative_regret"] = float(metrics0["worst_relative_regret"])
    ...
    for epoch in range(1, maximum_epochs + 1):
        ...
        if len(batches) != 4 or batch_size != 4:
            _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "expected 4 batches of size 4")
        ...
        metrics = calibration_metrics_uniform_relative(model, calibration, device=device_t)
        ...
        record = v1_train.scientific_epoch_record(
            epoch=epoch,
            primary=metrics["worst_relative_regret"],
            secondary=metrics["macro_kl"],
            total_loss=sum(epoch_losses) / len(epoch_losses),
            global_optimizer_step=schedule.global_optimizer_step,
            sample_ids=flat_order,
        )
        record["worst_relative_regret"] = float(metrics["worst_relative_regret"])
    ...
    return {
        ...
        "best_epoch": selector.best_epoch,
        "seed": seed,
        "worst_relative_regret": selector.best_worst,
        ...
        "last_model_state_scientific_sha256": v3.model_state_scientific_sha256(
            _cpu_identity_model_v4(model)
        ),
        "last_epoch": last_epoch,
        "global_optimizer_step": schedule.global_optimizer_step,
        "epoch0_worst_relative_regret": float(metrics0["worst_relative_regret"]),
