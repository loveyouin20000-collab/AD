"""B2-05C2 V3 evaluation gates (thresholds identical to C1/V2)."""
...
class B2DLCMV3EvaluationError(RuntimeError):
...
def _fail(code: str, detail: str) -> NoReturn:
    raise B2DLCMV3EvaluationError(code, detail)
...
def c1_comparison_diagnostic(
    *,
    v3_metrics: Mapping[str, float],
    c1_metrics: Mapping[str, float],
) -> dict[str, Any]:
    """Non-blocking C1/C2 comparison diagnostic (never a qualification gate)."""

    deltas = {
        key: float(v3_metrics.get(key, 0.0)) - float(c1_metrics.get(key, 0.0))
        for key in sorted(set(v3_metrics) | set(c1_metrics))
    }
    return {
...
def build_h_decision(
    *,
    ...
    payload = {
        "schema_version": "b2_dlcm_v3_h_decision_v1",
