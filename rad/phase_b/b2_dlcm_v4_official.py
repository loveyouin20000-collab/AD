"""B2-05C3B official V4 uniform-relative DLCM training orchestration."""
...
from rad.phase_b import b2_dlcm_v4_roster_adoption as roster_adoption
from rad.phase_b import b2_dlcm_v4_training as v4_training

OFFICIAL_PLAN_SCHEMA_VERSION = "b2_dlcm_v4_accepted_training_plan_v1"
C3A_CONTRACT_TAG = "b2-dlcm-uniform-relative-contract-v4"
C3A_ADOPTION_COMMIT = "3b2237affbe58a3cdc30d49bbdee4d8145a6a192"
V4_IMPLEMENTATION_COMMIT = "aee4eec9fe17d1972195de2f70a91ceaadd5a28f"


class B2DLCMV4OfficialError(RuntimeError):
...
def _fail(code: str, detail: str) -> NoReturn:
    raise B2DLCMV4OfficialError(code, detail)
...
def scientific_v4_training_plan_payload(
    *,
    ...
    """Accepted V3 plan — excludes operational/runtime fields per C3B contract."""
    ...
    return {
        ...
        "architecture_contract_version": v4.ARCHITECTURE_CONTRACT_VERSION,
        "model_class_id": v4.MODEL_CLASS_ID,
        "v4_implementation_commit": V4_IMPLEMENTATION_COMMIT,
        "c3a_adoption_commit": C3A_ADOPTION_COMMIT,
        "c3a_contract_tag": C3A_CONTRACT_TAG,
        ...
        "losses": {
            ...
            "relative_regret_mode": "batch_matched_model_minus_uniform",
            "subtract_gate_slack": False,
            "clamp_negative_regret": False,
            "absolute_regret": False,
            "smoothmax_tau": float(config.get("smoothmax_tau", v4.SMOOTHMAX_TAU)),
            ...
            "selection": "constrained_worst_relative_regret",
        },
        "sampler": {
            "sampler_contract_version": str(
                config.get("sampler_contract_version", v4_training.SAMPLER_CONTRACT_VERSION)
            ),
        ...
        "artifact_schema_versions": {
            ...
            "v4_protocol": protocol.SCHEMA_VERSION,
        },
        "v1_immutable": v4.v1_immutable_identity(),
        "v2_immutable": v4.v2_immutable_identity(),
        "v3_immutable": v4.v3_immutable_identity(),
    }


def compute_accepted_v4_training_plan_scientific_sha256(
    *,
    config: Mapping[str, Any],
    verified: v1_training.VerifiedB2DLCMTrainingInputs,
    roster_payload: Mapping[str, Any],
    ...
    payload = scientific_v4_training_plan_payload(
        config=config,
        verified=verified,
...
def load_frozen_roster_and_adoption(repo_root: Path | str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    adoption_path = root / "docs/phase_b/b2_05c3_final_roster_adoption_manifest.json"
    ...
    if adoption_payload.get("implementation_commit") != V4_IMPLEMENTATION_COMMIT:
        _fail(
            "B2_DLCM_V4_CONTRACT_MISMATCH",
            "adoption implementation_commit != V4_IMPLEMENTATION_COMMIT",
        )
...
def verify_repository_identity_gate(
    *,
    ...
    if not bool(config.get("repository_identity_gate_enabled", True)):
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "repository identity gate must be enabled")
    ...
    if status.returncode != 0:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", status.stderr.strip() or "git status failed")
    if status.stdout.strip():
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "worktree must be clean for official training")
    head = _git("rev-parse", "HEAD").stdout.strip()
    tag_proc = _git("rev-parse", f"{expected_tag}^{{commit}}")
    if tag_proc.returncode != 0:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", f"missing local tag {expected_tag}")
    ...
    if expected_impl != V4_IMPLEMENTATION_COMMIT:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "expected_implementation_commit pin mismatch")
    ancestor = _git("merge-base", "--is-ancestor", expected_commit, head)
    if ancestor.returncode != 0:
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "HEAD does not descend from contract commit")
    return {
        "head": head,
...
def require_plan_sha_agreement(
    *,
    ...
    pinned = config.get("expected_accepted_v4_training_plan_sha256")
    if pinned is not None and pinned != recomputed:
        _fail(
            "B2_DLCM_V4_CONTRACT_MISMATCH",
            "config.expected_accepted_v4_training_plan_sha256 != recomputed plan",
        )
...
def official_v4_dry_run(
    *,
    ...
    """Verify real upstream + C3A identities + plan hash; no model/RNG/writes/eval."""

    protocol.reject_bypass_flags(config)
    if config.get("contract_stage") != "b2_05c3b":
        _fail("B2_DLCM_V4_CONTRACT_MISMATCH", "official dry-run requires b2_05c3b")
    if config.get("real_training_enabled") is not True:
        _fail("B2_DLCM_V4_REAL_TRAINING_NOT_ENABLED", "dry-run requires real_training_enabled=true")
    ...
    if abs(float(config.get("smoothmax_tau", -1)) - v4.SMOOTHMAX_TAU) > 0:
    ...
    plan_sha = compute_accepted_v4_training_plan_scientific_sha256(
        config=config,
        verified=verified,
        roster_payload=roster_payload,
        adoption_payload=adoption_payload,
...
def run_official_v4_seed_training(
    *,
    output_root: Path,
    seed: int,
    ...
    patience: int = 50,
    batch_size: int = 4,
    smoothmax_tau: float = v4.SMOOTHMAX_TAU,
    device: str = "cuda",
) -> dict[str, Any]:
    eval_placeholders = [
    ...
    result = v4_training.run_v4_contract_training(
        output_root=Path(output_root),
        seed=seed,
        records=records,
