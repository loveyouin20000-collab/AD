...
from rad.phase_b import b2_dlcm_v4_official as official  # noqa: E402
...
def main() -> int:
    ...
    summary = official.official_v4_dry_run(
        config=config,
    ...
    for field in (
        ...
        "batches_per_epoch",
        "batch_composition",
        "relative_regret_mode",
        "calibration_records",
        "development_records_declared",
        "final_roster_records_declared",
        "final_records_loaded",
        "accepted_v4_training_plan_scientific_sha256",
    ):
        value = summary[field]
        rendered = str(value).lower() if isinstance(value, bool) else value
        print(f"{field} = {rendered}")
    print("B2_DLCM_V4_PLAN_RESULT=" + json.dumps(summary, sort_keys=True, separators=(",", ":")))
    return 0
