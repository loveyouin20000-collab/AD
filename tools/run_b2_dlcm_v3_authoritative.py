#!/usr/bin/env python3
"""Authoritative B2-05C2B three-seed V3 category-robust DLCM training + development gates."""
...
def main() -> int:
    ...
    from rad.phase_b import b2_dlcm_v3_deployment as v3_deployment
    from rad.phase_b import b2_dlcm_v3_evaluation as v3_evaluation
    from rad.phase_b import b2_dlcm_v3_official as official
    ...
    plan_sha = official.compute_accepted_v3_training_plan_scientific_sha256(
    ...
    if not args.skip_training:
        for seed in list(config["seeds"]):
            ...
            result = official.run_official_v3_seed_training(
            ...
            seed_scientific = training._canonical_json_sha256(
                {
                    ...
                    "worst_category_kl": float(result["worst_category_kl"]),
            ...
            seed_manifest = {
                "schema_version": "b2_dlcm_v3_seed_manifest_v1",
                "seed": int(seed),
                "status": "passed",
                "best_epoch": result["best_epoch"],
                ...
                "epoch0_worst_category_kl": float(result["epoch0_worst_category_kl"]),
                ...
                json.dumps(
                    {
                        ...
                        "best_epoch": result["best_epoch"],
                        "worst_category_kl": result["worst_category_kl"],
            ...
            seed_rows.append(
                {
                    ...
                    "worst_category_kl": float(man["worst_category_kl"]),
    ...
    worsts = [r["worst_category_kl"] for r in seed_rows]
    ...
    collection["worst_category_kl_mean"] = mean_w
    collection["worst_category_kl_std_ddof0"] = std_w
    ...
    # Constrained worst-category canonical selection
    canon_seed = v3_training.select_canonical_seed_category_robust(
        [
            {
                "seed": r["seed"],
                "worst_category_kl": r["worst_category_kl"],
                "macro_kl": r["macro_kl"],
    ...
    selection = {
        "schema_version": "b2_dlcm_v3_canonical_seed_selection_v1",
        ...
        "rule": "eligible_worst_category_macro_signed_min_seed",
        "min_delta": 1e-5,
    ...
    if not repro_root.exists():
        ...
        repro = official.run_official_v3_seed_training(
    ...
    model = v3.B2DLCMV3(seed=canon_seed)
    model.load_state_dict(orig_ckpt["model"], strict=True)
    model_repro = v3.B2DLCMV3(seed=canon_seed)
    model_repro.load_state_dict(repro_ckpt["model"], strict=True)
    deploy_a = v3.extract_deployment_state_dict(model)
    deploy_b = v3.extract_deployment_state_dict(model_repro)
    ...
    deploy_ckpt = v3_deployment.export_v3_deployment_checkpoint(
        model,
    ...
    gate = v3_evaluation.evaluate_development_gates(
    ...
    aux = v3_evaluation.build_auxiliary_diagnostics_manifest(
        diagnostics={
    ...
    protocol.persist_json_atomic(
        ...
        {
            "schema_version": "b2_dlcm_v3_development_evaluation_v1",
    ...
    # Non-blocking C1/C2 diagnostic comparison (if C1 summary is available)
    c1_comparison = {"available": False}
    c1_summary_candidates = [
        Path("/root/autodl-tmp/AD-phase-b2-dlcm-decoupled-canonical-training/artifacts/phase_b/b2_dlcm_decoupled_training/authoritative-run-20260805-023347/authoritative_run_summary.json"),
    ]
    for cand in c1_summary_candidates:
        if cand.is_file():
            c1 = json.loads(cand.read_text(encoding="utf-8"))
            c1_gate = c1.get("development_gates", {})
            c1_comparison = {
                "available": True,
                "c1_summary_path": str(cand),
                "c1_canonical_seed": c1.get("canonical_seed"),
                "c1_development_verdict": c1.get("development_verdict"),
                "c2_macro_gt_kl": float(target["kl_gt_dlcm"]["category_macro"]),
                "c2_per_category_gt_kl": {
                    k: float(v) for k, v in target["kl_gt_dlcm"]["per_category"].items()
                },
                "c2_worst_category_gt_kl": max(
                    float(v) for v in target["kl_gt_dlcm"]["per_category"].values()
                ),
                "note": "diagnostic_only_non_blocking",
            }
            break
    training._atomic_write_json(output_root / "c1_c2_diagnostic_comparison.json", c1_comparison)
    ...
    summary = {
        ...
        "worst_category_kl_mean": mean_w,
        "worst_category_kl_std_ddof0": std_w,
        "macro_kl_mean": mean_m,
        "macro_kl_std_ddof0": std_m,
        "c1_c2_diagnostic_comparison": c1_comparison,
