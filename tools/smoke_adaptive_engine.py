...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Synthetic smoke test for adaptive engine (not paper evaluation)"
    )
...
def main() -> int:
    ...
    output_dir = args.output_dir or Path(
        adaptive.get("smoke_output_dir", adaptive.get("output_dir", "artifacts/eval/smoke_adaptive"))
    )
    ...
    limit = args.limit if args.limit is not None else adaptive.get("smoke_limit", adaptive.get("limit"))
    ...
    for i in tqdm(range(limit_n), desc="smoke_adaptive_engine"):
