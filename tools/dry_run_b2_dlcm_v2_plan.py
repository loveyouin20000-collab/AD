#!/usr/bin/env python3
"""Dual-process dry-run helper for accepted V2 training plan SHA."""
...
from rad.phase_b import b2_dlcm_v2_official as official  # noqa: E402
...
def main() -> int:
    ...
    summary = official.official_v2_dry_run(
        config=config,
    ...
    for field in (
        ...
        "accepted_v2_training_plan_scientific_sha256",
    ):
        value = summary[field]
        rendered = str(value).lower() if isinstance(value, bool) else value
        print(f"{field} = {rendered}")
    print("B2_DLCM_V2_PLAN_RESULT=" + json.dumps(summary, sort_keys=True, separators=(",", ":")))
    return 0
